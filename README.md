# 👋 Hey there! I'm GREGORY STEPHEN OTIENO
#### 💻 Software Engineering Student

### 🔗 Connect With Me

[![Email](https://img.shields.io/badge/Email-gregorysteve656@gmail.com-D14836?style=flat&logo=gmail&logoColor=white)](mailto:gregorysteve656@gmail.com)
[![WhatsApp](https://img.shields.io/badge/WhatsApp-25D366?style=flat&logo=whatsapp&logoColor=white)](https://wa.me/254719637416)
[![Portfolio](https://img.shields.io/badge/Portfolio-gregory--steve.web.app-blue?style=flat&logo=firebase&logoColor=white)](https://gregory-steve.web.app/)
[![Instagram](https://img.shields.io/badge/Instagram-%40reddevcode-E4405F?style=flat&logo=instagram&logoColor=white)](https://www.instagram.com/reddevcode)

---
### 🧠 The Developer Behind The Code

<details>
<summary>Click to view my technical summary</summary>

```javascript
const whoami = {
  name: "GREGORY STEPHEN OTIENO",
  role: "Software Engineering Student",
  location: "Nairobi, Kenya",
  skills: [
    "Frontend: [React, Next.js, Vue, Svelte, Bootstrap, TailwindCSS, CSS Frameworks]",
    "Backend: [Node.js, Python, Firebase, and related NoSQL/RDBMS]",
    "APIs: [OpenAI API, REST Integration, Google Cloud Services, Data Management]",
  ],
  currentFocus: [
    "#1: Scaling and optimizing key projects like EdupulseAI and SmartFarm.",
    "#2: Building high-impact solutions that directly address African market challenges.",
    "#3: Expanding knowledge in robust systems design and security protocols.",
  ],
  mindset: "Code + Innovation = Impact"
};
